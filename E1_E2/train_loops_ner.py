...
def test_model(model, test_loader, dat_kind='Test'): #saves the final metrics for input data 
    pred_tags = []
    target_tags = []
    model.eval()
    ...
    for input, target in test_loader:
        ...
        for j in range(output.shape[0]):
            ...
            for k in range(target.shape[1]):
                if target[j,k]==-1: #i.e. not no word
                    break
...
def test_model_output(model, test_file, output_file, dat_kind='Test'): #saves the final metrics for input data 
    ...
    data_file = '\n\n'.join(data_file)
    text_file = open(FOLDER + '/' + output_file, "w")
...
def train_model_crf(model, train_loader, val_loader):
    ...
    for epoch in range(start_epoch, epochs+1):
      ...
      out = torch.empty(0, device=device)
      model.train()
      #scaler = torch.cuda.amp.GradScaler() #1
      for i, (input, target) in enumerate(train_loader):
          input, target = input.to(device), target.to(device)
          optimizer.zero_grad()
          target = target.long()
...
def test_model_crf(model, test_file, output_file, dat_kind='Test'): #saves the final metrics for input data 
    test_loader = get_loader_test(batch_size, test_file)
    pred_tags = []
    target_tags = []
    data_file = open(test_file).read().split('\n\n')[:-1]
